...
/** Rule to detect usage of Option.get. Calling .get on an Option (or Try)
  * throws an exception if the value is missing/failed. Note: This rule is
  * syntactic and flags any parameterless .get call. Use .getOrElse, .fold, or
  * .map instead.
...
object AvoidOptionGet extends Rule {
  ...
  override def check(tree: Tree): Seq[Issue] = {
    tree.collect {
      case t @ Term.Select(_, Term.Name("get")) if !t.parent.exists {
            case Term.Apply(_, args) => args.nonEmpty
            case _                   => false
          } =>
        Issue(
          "Style: Avoid using .get (unsafe on Option/Try). It throws exceptions on empty/failure. Use .getOrElse(), .fold(), or .map() instead.",
          t.pos
        )
    }
  }
