...
/**
 * Rule to detect deeply nested if-else statements.
 * Deep nesting increases cognitive load and cyclomatic complexity. Suggests refactoring or using pattern matching.
 */
  ...
  override def check(tree: Tree): Seq[Issue] = {
    tree.collect {
      case t: Term.If if calculateDepth(t) > MaxDepth =>
        Issue(s"Style: Avoid nested if-else statements deeper than $MaxDepth layers. Consider refactoring or using pattern matching.", t.pos)
    }
  }
