...
/** Rule to detect catching 'Throwable'. Catching Throwable intercepts fatal
  * errors (OOM, StackOverflow) which leaves the JVM in a zombie state. Use
  * 'case NonFatal(e) =>' instead.
  ...
  override def check(tree: Tree): Seq[Issue] = {
    tree.collect {
      case c @ Case(Pat.Typed(_, Type.Name("Throwable")), _, _) =>
        Issue(
          "Reliability: Do not catch 'Throwable'. This catches fatal errors like OutOfMemoryError. Use 'import scala.util.control.NonFatal' and 'case NonFatal(e) =>' instead.",
          c.pos
        )
