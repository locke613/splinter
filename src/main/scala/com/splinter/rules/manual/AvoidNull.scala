...
/**
 * Rule to detect usage of 'null'.
 * Null values are error-prone and invisible to the type system.
...
object AvoidNull extends Rule {
  ...
  override def check(tree: Tree): Seq[Issue] = {
    tree.collect {
      case t: Lit.Null =>
        Issue("Style: Avoid using 'null'. Use 'Option[T]' for optional values to prevent NullPointerException.", t.pos)
    }
  }
}