...
/** Rule to detect updates to vars inside loops or conditions. This encourages
  * expression-oriented programming (val result = if...) and avoids Spark
  ...
  override def check(tree: Tree): Seq[Issue] = {
    tree.collect {
      case t @ Term.Assign(_, _) if isInsideLoopOrCondition(t) =>
        Issue(
          "Style: Avoid updating vars inside loops or conditions. Use functional transformations (map/fold) or 'val result = if...' expressions.",
          t.pos
        )
      ...
      case t @ Term.ApplyInfix(_, Term.Name(op), _, _)
        ...
        Issue(
          s"Style: Avoid updating vars inside loops or conditions (detected '$op').",
          t.pos
        )
    }
  }
      ...
      case Some(node) =>
        node match {
          ...
          case Term.Apply(
                Term.Select(
                  _,
                  Term.Name("foreach") | Term.Name("map") | Term.Name("flatMap")
                ),
                _
              ) =>
            true
