...
/**
 * Rule to detect usage of Seq.head.
 * Calling .head on a Seq throws NoSuchElementException if the collection is empty.
 * Use .headOption or pattern matching instead.
  ...
  override def check(tree: Tree): Seq[Issue] = {
    tree.collect {
      case t @ Term.Select(_, Term.Name("head")) 
        ...
        Issue("Style: Avoid using .head. It throws NoSuchElementException if the collection is empty. Use .headOption instead.", t.pos)
    }
  }
}