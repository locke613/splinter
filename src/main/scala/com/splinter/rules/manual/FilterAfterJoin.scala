...
/** Rule to detect filters applied immediately after a join. Filtering before
  * joining reduces the dataset size involved in the shuffle, improving
  * performance.
  */
  ...
  override def check(tree: Tree): Seq[Issue] = {
    tree.collect {
      case term @ Term.Apply(
        ...
        Issue(
          "Performance: Filter detected after a join. Try to move filters before the join to reduce shuffle size.",
          term.pos
        )
    }
  }
